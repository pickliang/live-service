...
public class SmjjTest {
	...
	private List<OrderEntity> order(JSONObject item) {
		...
		for (Object object : orderList) {
			...
			orderEntity.setPhone(order.getString("custPhone"));
	...
	private List<RaiseEntity> raise(JSONObject item) {
		...
		for (Object object : raiseList) {
			...
			raiseEntity.setUpdateImage(raise.getInteger("isVideoConfirm"));
	...
	private  ProductEntity product(JSONObject item){
		...
		productEntity.setRateDetail(item.getString("subscriptionRemark"));
		if(item.getJSONArray("otherFee") !=null ) {
			...
			productEntity.setOtherRate(JSONObject.toJSONString(list));//组合
		}
