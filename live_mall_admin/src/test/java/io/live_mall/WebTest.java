...
public class WebTest {
	...
	@Test
	public  void parse() {
		SysConfigEntity sysConfigEntity = sysConfigService.getById("3");
	...
	private List<OrderEntity> order(JSONObject item) {
		...
		for (Object object : orderList) {
			...
			orderEntity.setCardPhotoR(order.getString("custCertificationFront"));
			orderEntity.setCardPhotoL(order.getString("custCertificationBack"));
			orderEntity.setCardTimeType(order.getString("custCertificationPeriodForever"));
			...
			orderEntity.setBankCardFront(order.getString("bankCardFront"));
			orderEntity.setBankCardBack(order.getString("bankCardBack"));
			
			orderEntity.setOpenAccount(order.getString("applicationFormOpenAccount"));
			orderEntity.setSubAgree(order.getString("subscriptionAgreement"));
			orderEntity.setPaymentSlip(order.getString("paymentVoucher"));
			orderEntity.setAssetsPro(order.getString("custAssets"));
			orderEntity.setOtherFile(order.getString("otherAccessories"));
		...
		if(unit !=null) {
			...
			if(unit.getJSONObject("duration") !=null){
				...
				if(duration.getInteger("type") == 1) {
					...
					if("1".equals(item.getString("afterFoundedDateUnit"))) {
						productUnit.setDeadlineData(duration.getString("afterFoundedDateNumber")+","+"年");
					}else if("2".equals(item.getString("afterFoundedDateUnit"))) {
						productUnit.setDeadlineData(duration.getString("afterFoundedDateNumber")+","+"月");
					}else if("3".equals(item.getString("afterFoundedDateUnit"))) {
				...
				if(duration.getInteger("type") == 5) {
					...
					if("1".equals(item.getString("afterFoundedDateMaxUnit"))) {
						productUnit.setDeadlineData(duration.getString("afterFoundedDateMaxNumber")+","+"年");
					}else if("2".equals(item.getString("afterFoundedDateMaxUnit"))) {
						productUnit.setDeadlineData(duration.getString("afterFoundedDateMaxNumber")+","+"月");
					}else if("3".equals(item.getString("afterFoundedDateMaxUnit"))) {
	...
	private  ProductEntity product(JSONObject item){
		...
		if(item.getIntValue("category") == 4){
			productEntity.setOnetype("资管计划");
		}*/
		
		productEntity.setProductName(item.getString("name"));
		productEntity.setProductBrif(item.getString("shortName"));
		/**
		 * <li data-text="私募股权类" data-value="11">私募股权类</li>
		 * <li data-text="私募证券类" data-value="12" class="w-selected">私募证券类</li>
		 * <li data-text="私募其他类" data-value="13">私募其他类</li>
		 * <li data-text="资产配置类" data-value="14">资产配置类</li>
		 */
		...
		productEntity.setCurrencyType(item.getString("currencyType"));
		...
		productEntity.setRiskLevel(item.getString("riskLevel"));
		...
		if(object.getString("type").equals("2")) {
			...
			if("1".equals(item.getString("alterableUnit"))) {
			...
			}else if("2".equals(item.getString("alterableUnit"))) {
			...
			}else if("3".equals(item.getString("alterableUnit"))) {
		...
		if(openDay.getString("type").equals("2")){
			...
			productEntity.setTermTwo(item.getString("periodicallyMonth")+","+item.getString("periodicallyDate"));
			productEntity.setOpenHouseType("1");
		}else if(openDay.getString("type").equals("4")){
			productEntity.setTermTwo(item.getString("scheduledDate"));
		...
		productEntity.setInvestmentConsultant(item.getString("consultant"));
		...
		productEntity.setElectroniceContract(item.getString("contract"));
		...
		productEntity.setBuyRate(item.getString("otherAttachment"));
	...
	public void orderName() throws Exception {
		List<OrderEntity> list = orderService.list(new QueryWrapper<OrderEntity>().in("status", 2,4));
	     list.stream().forEach(order->{
	    	 MemberEntity memberEntity = memberService.saveOrUpdate(order);
